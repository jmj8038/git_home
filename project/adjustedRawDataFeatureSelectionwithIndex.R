a <- merge_train_data_adjusted_index
summary(a)
length(is.na(a))
colnames(a)
nrow(a)
...
training_index <- createDataPartition(h$transaction_real_price, p=0.8, list = FALSE)
real_t <- h[training_index, ]
real_v <- h[-training_index, ]
...
write.csv(h, file = "realestateDataForAnalysis.csv")
...
rawData = h #read.csv("realestatedForRegression-1.csv")
